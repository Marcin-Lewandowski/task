...
def get_items():                        # funkcja wyświetla listę towarów w magazynie
    ...
    for i in items:
        if len(i['name']) > 7:
            print("%s\t\t%.2f\t\t%s\t%.2f\t\t\t%s" % (
                i["name"], i["quantity"], i["unit"], i["unit_price"], i["date_and_time"]))
...
def lista_sprzedanych_towarów():        # wyświetla listę sprzedanych towarów
    ...
    for x in sold_items:  # x - jest słownikiem sprzedanego towaru
        if len(x['name']) > 7:
            ...
            print("%s\t\t%d\t\t%s\t%.2f\t\t\t%s\t%s" % (
                x["name"], x["quantity"], x["unit"], x["unit_price"], x["date_and_time"], x['klient']))
        else:
            print("%s\t\t\t%d\t\t%s\t%.2f\t\t\t%s\t%s" % (
                x["name"], x["quantity"], x["unit"], x["unit_price"], x["date_and_time"], x['klient']))


# funkcja sprawdza czy dany towar (item_name) istnieje w magazynie ( lista items)
...
# dodanie nowego słownika do listy items, trzeba udoskonalić bo dodaje kolejny towar o takiej samej nazwie a tak nie może być
def add_item(item_name, item_quantity, item_unit, item_price):
...
# funkcja odejmuje ze stanu magazynowego towar np. milk w ilości określonej przez wprowadzoną liczbę
def sell_item(nazwa_towaru, sprzedana_ilosc, ncs, nazwa_klienta):
    now = datetime.now()
            ...
            if wartosc == nazwa_towaru:
                if slownik["quantity"] >= sprzedana_ilosc:
                    ...
                    sold_items.append({
                        ...
                        "unit": slownik["unit"],
                        # * slownik['cena_sprzedazy'] może trzeba bedzie podac cene sprzedazy jako argument funkcji
                        "unit_price": ncs,
                        "date_and_time": now.strftime("%d/%m/%Y    %H:%M"),
                    ...
                    print("Successfully sold %d %s of %s firmie %s" % (
                        sprzedana_ilosc, slownik["unit"], nazwa_towaru, nazwa_klienta))
...
# funkcja eksportuje / zapisuje do pliku magazyn.csv całą zawartość listy items.
def export_items_to_csv():

    with open('magazyn.csv', 'w', newline='', encoding="utf-8") as csvfile:

        fieldnames = ['name', 'quantity', 'unit',
                      'unit_price', 'date_and_time']
...
# funkcja eksportuje / zapisuje do pliku magazyn_sprzedaz.csv całą zawartość listy sold_items.
def export_sales_to_csv():

    with open('magazyn_sprzedaz.csv', 'w', newline='', encoding="utf-8") as csvfile:

        fieldnames = ['name', 'quantity', 'unit',
                      'unit_price', 'date_and_time', 'klient']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
